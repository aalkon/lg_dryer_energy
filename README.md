...
### Idempotency & Flap Handling

LG ThinQ sensors routinely flap through `unknown` and `unavailable`. The integration guards against double-attribution in three layers:
...
This file is automatically managed. Sessions older than 14 days are garbage collected; sessions that missed their attribution window (e.g., because HA was down on the morning the LG cloud push would have fired) are logged and dropped on the next successful attribution. You can safely delete this file to reset the integration's state — the cumulative sum will restart from whatever the statistics database already has.
